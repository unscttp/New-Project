...
def _build_tool_registry(tool_specs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for spec in tool_specs:
        ...
        if fn is None:
            raise KeyError(f"tool_registry.json 中引用了未实现工具: {name}")
        registry[name] = fn
    return registry
...
def _build_openai_tools(tool_specs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for spec in tool_specs:
        args_model_name = spec["args_model"]
        args_model = globals().get(args_model_name)
        if args_model is None:
            raise KeyError(f"tool_registry.json 中引用了未知参数模型: {args_model_name}")
        payload.append(
            {
                "type": "function",
                "function": {
